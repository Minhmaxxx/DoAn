...
- Nothing may construct a Supabase client just by rendering a page: `tests/test_pages.py` asserts every page stays guest-only, which is what keeps the release gate offline. `utils.auth` functions take `get_client().auth`; repositories take `get_client()`. See `STORAGE_PLAN.md` for the required Supabase settings (Anonymous sign-ins, Manual Linking, Redirect URLs). `storage_schema.sql` sets up the tables and RLS; `verify_rls.sql` confirms RLS is actually enabled in the live database, which nothing in the app can tell you — with the policies missing it behaves identically and the tables are simply open.
...
- Every page authors raw HTML through `st.markdown(..., unsafe_allow_html=True)`, so escaping is load-bearing. Any `{...}` interpolated into one of those blocks must either be formatted as a number (`{x:.0f}`), wrapped in `html.escape()`, or named in `tests/test_xss.py::PREBUILT_HTML_NAMES` with a comment saying who escapes its inputs — a static `ast` guard fails the gate otherwise. User text reaches these blocks two ways: the free-text profile name, which every page shows through `render_page_header()`, and meal records read back out of Postgres.
- `utils/history.build_export_payload()` / `export_as_json()` are pure functions over plain values — no Streamlit, no session state — and copy their inputs, because the document is handed to `st.download_button` and outlives the run. Bump `EXPORT_FORMAT_VERSION` when the meal-record shape changes.
