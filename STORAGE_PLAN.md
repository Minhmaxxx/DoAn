...
|---|---|---|
| 1a | **Đạt** (2026-08-21) | `verify_rls.sql`, cả ba câu. `relrowsecurity = true` trên cả `profiles` và `meals`; đủ 8 policy; `qual`/`with_check` đều so `auth.uid()` với `user_id`. Chạy lại mỗi khi áp lại schema hoặc khôi phục project sau pause |
